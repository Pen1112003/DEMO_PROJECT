# FR-037 | submit a parking issue report.

**Target requirement:** FR-037 — The system shall allow Parking User/Driver to submit a parking issue report.
...
1. Primary feature screen for FR-037 with list/detail/workflow panel.
...
|---|---|---|
| POST | `/api/fr-037/records` | Create new record/configuration/workflow entry. |
| GET | `/api/fr-037/records` | List records with filters and pagination. |
| GET | `/api/fr-037/records/{recordId}` | Get record detail. |
| PUT | `/api/fr-037/records/{recordId}` | Update record fields. |
| PATCH | `/api/fr-037/records/{recordId}/status` | Change status/transition state. |
...
### Happy Path
1. **Given** valid permissions and required data exist, **when** the primary actor executes FR-037 workflow successfully, **then** the system persists results and returns the updated output.
