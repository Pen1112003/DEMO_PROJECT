# FR-017 | guide vehicles to assigned floors or zones.

**Target requirement:** FR-017 — The system shall allow Parking Staff to guide vehicles to assigned floors or zones.
...
1. Primary feature screen for FR-017 with list/detail/workflow panel.
...
|---|---|---|
| POST | `/api/fr-017/records` | Create new record/configuration/workflow entry. |
| GET | `/api/fr-017/records` | List records with filters and pagination. |
| GET | `/api/fr-017/records/{recordId}` | Get record detail. |
| PUT | `/api/fr-017/records/{recordId}` | Update record fields. |
| PATCH | `/api/fr-017/records/{recordId}/status` | Change status/transition state. |
...
### Happy Path
1. **Given** valid permissions and required data exist, **when** the primary actor executes FR-017 workflow successfully, **then** the system persists results and returns the updated output.
