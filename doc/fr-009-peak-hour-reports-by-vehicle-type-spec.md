# FR-009 | peak-hour reports by vehicle type.

**Target requirement:** FR-009 — The system shall provide peak-hour reports by vehicle type.
...
1. Primary feature screen for FR-009 with list/detail/workflow panel.
...
|---|---|---|
| POST | `/api/fr-009/records` | Create new record/configuration/workflow entry. |
| GET | `/api/fr-009/records` | List records with filters and pagination. |
| GET | `/api/fr-009/records/{recordId}` | Get record detail. |
| PUT | `/api/fr-009/records/{recordId}` | Update record fields. |
| PATCH | `/api/fr-009/records/{recordId}/status` | Change status/transition state. |
...
### Happy Path
1. **Given** valid permissions and required data exist, **when** the primary actor executes FR-009 workflow successfully, **then** the system persists results and returns the updated output.
