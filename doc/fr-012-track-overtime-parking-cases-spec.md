# FR-012 | track overtime parking cases.

**Target requirement:** FR-012 — The system shall allow the Parking Manager to track overtime parking cases.
...
1. Primary feature screen for FR-012 with list/detail/workflow panel.
...
|---|---|---|
| POST | `/api/fr-012/records` | Create new record/configuration/workflow entry. |
| GET | `/api/fr-012/records` | List records with filters and pagination. |
| GET | `/api/fr-012/records/{recordId}` | Get record detail. |
| PUT | `/api/fr-012/records/{recordId}` | Update record fields. |
| PATCH | `/api/fr-012/records/{recordId}/status` | Change status/transition state. |
...
### Happy Path
1. **Given** valid permissions and required data exist, **when** the primary actor executes FR-012 workflow successfully, **then** the system persists results and returns the updated output.
