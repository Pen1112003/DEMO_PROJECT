# FR-023 | handle lost ticket exceptions.

**Target requirement:** FR-023 — The system shall allow Parking Staff to handle lost ticket exceptions.
...
1. Primary feature screen for FR-023 with list/detail/workflow panel.
...
|---|---|---|
| POST | `/api/fr-023/records` | Create new record/configuration/workflow entry. |
| GET | `/api/fr-023/records` | List records with filters and pagination. |
| GET | `/api/fr-023/records/{recordId}` | Get record detail. |
| PUT | `/api/fr-023/records/{recordId}` | Update record fields. |
| PATCH | `/api/fr-023/records/{recordId}/status` | Change status/transition state. |
...
### Happy Path
1. **Given** valid permissions and required data exist, **when** the primary actor executes FR-023 workflow successfully, **then** the system persists results and returns the updated output.
