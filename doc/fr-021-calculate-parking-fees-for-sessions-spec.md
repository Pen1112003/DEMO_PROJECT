# FR-021 | calculate parking fees for sessions.

**Target requirement:** FR-021 — The system shall allow Parking Staff to calculate parking fees for sessions.
...
1. Primary feature screen for FR-021 with list/detail/workflow panel.
...
|---|---|---|
| POST | `/api/fr-021/records` | Create new record/configuration/workflow entry. |
| GET | `/api/fr-021/records` | List records with filters and pagination. |
| GET | `/api/fr-021/records/{recordId}` | Get record detail. |
| PUT | `/api/fr-021/records/{recordId}` | Update record fields. |
| PATCH | `/api/fr-021/records/{recordId}/status` | Change status/transition state. |
...
### Happy Path
1. **Given** valid permissions and required data exist, **when** the primary actor executes FR-021 workflow successfully, **then** the system persists results and returns the updated output.
