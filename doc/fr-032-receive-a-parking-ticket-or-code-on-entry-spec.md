# FR-032 | receive a parking ticket or code on entry.

**Target requirement:** FR-032 — The system shall allow Parking User/Driver to receive a parking ticket or code on entry.
...
1. Primary feature screen for FR-032 with list/detail/workflow panel.
...
|---|---|---|
| POST | `/api/fr-032/records` | Create new record/configuration/workflow entry. |
| GET | `/api/fr-032/records` | List records with filters and pagination. |
| GET | `/api/fr-032/records/{recordId}` | Get record detail. |
| PUT | `/api/fr-032/records/{recordId}` | Update record fields. |
| PATCH | `/api/fr-032/records/{recordId}/status` | Change status/transition state. |
...
### Happy Path
1. **Given** valid permissions and required data exist, **when** the primary actor executes FR-032 workflow successfully, **then** the system persists results and returns the updated output.
