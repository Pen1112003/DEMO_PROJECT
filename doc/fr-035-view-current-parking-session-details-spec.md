# FR-035 | view current parking session details.

**Target requirement:** FR-035 — The system shall allow Parking User/Driver to view current parking session details.
...
1. Primary feature screen for FR-035 with list/detail/workflow panel.
...
|---|---|---|
| POST | `/api/fr-035/records` | Create new record/configuration/workflow entry. |
| GET | `/api/fr-035/records` | List records with filters and pagination. |
| GET | `/api/fr-035/records/{recordId}` | Get record detail. |
| PUT | `/api/fr-035/records/{recordId}` | Update record fields. |
| PATCH | `/api/fr-035/records/{recordId}/status` | Change status/transition state. |
...
### Happy Path
1. **Given** valid permissions and required data exist, **when** the primary actor executes FR-035 workflow successfully, **then** the system persists results and returns the updated output.
