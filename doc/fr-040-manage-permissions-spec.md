# FR-040 | manage permissions.

**Target requirement:** FR-040 — The system shall allow the System Administrator to manage permissions.
...
1. Primary feature screen for FR-040 with list/detail/workflow panel.
...
|---|---|---|
| POST | `/api/fr-040/records` | Create new record/configuration/workflow entry. |
| GET | `/api/fr-040/records` | List records with filters and pagination. |
| GET | `/api/fr-040/records/{recordId}` | Get record detail. |
| PUT | `/api/fr-040/records/{recordId}` | Update record fields. |
| PATCH | `/api/fr-040/records/{recordId}/status` | Change status/transition state. |
...
### Happy Path
1. **Given** valid permissions and required data exist, **when** the primary actor executes FR-040 workflow successfully, **then** the system persists results and returns the updated output.
