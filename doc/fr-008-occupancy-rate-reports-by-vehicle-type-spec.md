# FR-008 | occupancy rate reports by vehicle type.

**Target requirement:** FR-008 — The system shall provide occupancy rate reports by vehicle type.
...
1. Primary feature screen for FR-008 with list/detail/workflow panel.
...
|---|---|---|
| POST | `/api/fr-008/records` | Create new record/configuration/workflow entry. |
| GET | `/api/fr-008/records` | List records with filters and pagination. |
| GET | `/api/fr-008/records/{recordId}` | Get record detail. |
| PUT | `/api/fr-008/records/{recordId}` | Update record fields. |
| PATCH | `/api/fr-008/records/{recordId}/status` | Change status/transition state. |
...
### Happy Path
1. **Given** valid permissions and required data exist, **when** the primary actor executes FR-008 workflow successfully, **then** the system persists results and returns the updated output.
