# FR-027 | update slot status after exceptions.

**Target requirement:** FR-027 — The system shall allow Parking Staff to update slot status after exceptions.
...
1. Primary feature screen for FR-027 with list/detail/workflow panel.
...
|---|---|---|
| POST | `/api/fr-027/records` | Create new record/configuration/workflow entry. |
| GET | `/api/fr-027/records` | List records with filters and pagination. |
| GET | `/api/fr-027/records/{recordId}` | Get record detail. |
| PUT | `/api/fr-027/records/{recordId}` | Update record fields. |
| PATCH | `/api/fr-027/records/{recordId}/status` | Change status/transition state. |
...
### Happy Path
1. **Given** valid permissions and required data exist, **when** the primary actor executes FR-027 workflow successfully, **then** the system persists results and returns the updated output.
