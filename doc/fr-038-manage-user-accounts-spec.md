# FR-038 | manage user accounts.

**Target requirement:** FR-038 — The system shall allow the System Administrator to manage user accounts.
...
1. Primary feature screen for FR-038 with list/detail/workflow panel.
...
|---|---|---|
| POST | `/api/fr-038/records` | Create new record/configuration/workflow entry. |
| GET | `/api/fr-038/records` | List records with filters and pagination. |
| GET | `/api/fr-038/records/{recordId}` | Get record detail. |
| PUT | `/api/fr-038/records/{recordId}` | Update record fields. |
| PATCH | `/api/fr-038/records/{recordId}/status` | Change status/transition state. |
...
### Happy Path
1. **Given** valid permissions and required data exist, **when** the primary actor executes FR-038 workflow successfully, **then** the system persists results and returns the updated output.
