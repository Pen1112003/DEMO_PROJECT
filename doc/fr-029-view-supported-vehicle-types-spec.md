# FR-029 | view supported vehicle types.

**Target requirement:** FR-029 — The system shall allow Parking User/Driver to view supported vehicle types.
...
1. Primary feature screen for FR-029 with list/detail/workflow panel.
...
|---|---|---|
| POST | `/api/fr-029/records` | Create new record/configuration/workflow entry. |
| GET | `/api/fr-029/records` | List records with filters and pagination. |
| GET | `/api/fr-029/records/{recordId}` | Get record detail. |
| PUT | `/api/fr-029/records/{recordId}` | Update record fields. |
| PATCH | `/api/fr-029/records/{recordId}/status` | Change status/transition state. |
...
### Happy Path
1. **Given** valid permissions and required data exist, **when** the primary actor executes FR-029 workflow successfully, **then** the system persists results and returns the updated output.
