# FR-015 | validate entry eligibility.

**Target requirement:** FR-015 — The system shall allow Parking Staff to validate entry eligibility.
...
1. Primary feature screen for FR-015 with list/detail/workflow panel.
...
|---|---|---|
| POST | `/api/fr-015/records` | Create new record/configuration/workflow entry. |
| GET | `/api/fr-015/records` | List records with filters and pagination. |
| GET | `/api/fr-015/records/{recordId}` | Get record detail. |
| PUT | `/api/fr-015/records/{recordId}` | Update record fields. |
| PATCH | `/api/fr-015/records/{recordId}/status` | Change status/transition state. |
...
### Happy Path
1. **Given** valid permissions and required data exist, **when** the primary actor executes FR-015 workflow successfully, **then** the system persists results and returns the updated output.
