# FR-011 | flag license plate mismatches.

**Target requirement:** FR-011 — The system shall allow the Parking Manager to flag license plate mismatches.
...
| Role | Permissions |
|---|---|
| Parking Manager | Primary role to execute this functional requirement. |
...
1. Primary feature screen for FR-011 with list/detail/workflow panel.
...
|---|---|---|
| POST | `/api/fr-011/records` | Create new record/configuration/workflow entry. |
| GET | `/api/fr-011/records` | List records with filters and pagination. |
| GET | `/api/fr-011/records/{recordId}` | Get record detail. |
| PUT | `/api/fr-011/records/{recordId}` | Update record fields. |
| PATCH | `/api/fr-011/records/{recordId}/status` | Change status/transition state. |
...
### Happy Path
1. **Given** valid permissions and required data exist, **when** the primary actor executes FR-011 workflow successfully, **then** the system persists results and returns the updated output.
