# FR-034 | reserve a slot in advance when reservation is enabled.

**Target requirement:** FR-034 — The system shall allow Parking User/Driver to reserve a slot in advance when reservation is enabled.
...
| Role | Permissions |
|---|---|
| Parking User/Driver | Primary role to execute this functional requirement. |
...
1. Primary feature screen for FR-034 with list/detail/workflow panel.
...
|---|---|---|
| POST | `/api/fr-034/records` | Create new record/configuration/workflow entry. |
| GET | `/api/fr-034/records` | List records with filters and pagination. |
| GET | `/api/fr-034/records/{recordId}` | Get record detail. |
| PUT | `/api/fr-034/records/{recordId}` | Update record fields. |
| PATCH | `/api/fr-034/records/{recordId}/status` | Change status/transition state. |
...
### Happy Path
1. **Given** valid permissions and required data exist, **when** the primary actor executes FR-034 workflow successfully, **then** the system persists results and returns the updated output.
