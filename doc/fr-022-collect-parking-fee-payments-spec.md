# FR-022 | collect parking fee payments.

**Target requirement:** FR-022 — The system shall allow Parking Staff to collect parking fee payments.
...
1. Primary feature screen for FR-022 with list/detail/workflow panel.
...
|---|---|---|
| POST | `/api/fr-022/records` | Create new record/configuration/workflow entry. |
| GET | `/api/fr-022/records` | List records with filters and pagination. |
| GET | `/api/fr-022/records/{recordId}` | Get record detail. |
| PUT | `/api/fr-022/records/{recordId}` | Update record fields. |
| PATCH | `/api/fr-022/records/{recordId}/status` | Change status/transition state. |
...
### Happy Path
1. **Given** valid permissions and required data exist, **when** the primary actor executes FR-022 workflow successfully, **then** the system persists results and returns the updated output.
