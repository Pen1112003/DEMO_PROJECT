# FR-010 | track lost ticket cases.

**Target requirement:** FR-010 — The system shall allow the Parking Manager to track lost ticket cases.
...
1. Primary feature screen for FR-010 with list/detail/workflow panel.
...
|---|---|---|
| POST | `/api/fr-010/records` | Create new record/configuration/workflow entry. |
| GET | `/api/fr-010/records` | List records with filters and pagination. |
| GET | `/api/fr-010/records/{recordId}` | Get record detail. |
| PUT | `/api/fr-010/records/{recordId}` | Update record fields. |
| PATCH | `/api/fr-010/records/{recordId}/status` | Change status/transition state. |
...
### Happy Path
1. **Given** valid permissions and required data exist, **when** the primary actor executes FR-010 workflow successfully, **then** the system persists results and returns the updated output.
