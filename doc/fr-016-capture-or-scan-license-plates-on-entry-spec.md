# FR-016 | capture or scan license plates on entry.

**Target requirement:** FR-016 — The system shall allow Parking Staff to capture or scan license plates on entry.
...
1. Primary feature screen for FR-016 with list/detail/workflow panel.
...
|---|---|---|
| POST | `/api/fr-016/records` | Create new record/configuration/workflow entry. |
| GET | `/api/fr-016/records` | List records with filters and pagination. |
| GET | `/api/fr-016/records/{recordId}` | Get record detail. |
| PUT | `/api/fr-016/records/{recordId}` | Update record fields. |
| PATCH | `/api/fr-016/records/{recordId}/status` | Change status/transition state. |
...
### Happy Path
1. **Given** valid permissions and required data exist, **when** the primary actor executes FR-016 workflow successfully, **then** the system persists results and returns the updated output.
