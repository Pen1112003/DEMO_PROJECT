# FR-020 | record exit times for sessions.

**Target requirement:** FR-020 — The system shall allow Parking Staff to record exit times for sessions.
...
1. Primary feature screen for FR-020 with list/detail/workflow panel.
...
|---|---|---|
| POST | `/api/fr-020/records` | Create new record/configuration/workflow entry. |
| GET | `/api/fr-020/records` | List records with filters and pagination. |
| GET | `/api/fr-020/records/{recordId}` | Get record detail. |
| PUT | `/api/fr-020/records/{recordId}` | Update record fields. |
| PATCH | `/api/fr-020/records/{recordId}/status` | Change status/transition state. |
...
### Happy Path
1. **Given** valid permissions and required data exist, **when** the primary actor executes FR-020 workflow successfully, **then** the system persists results and returns the updated output.
