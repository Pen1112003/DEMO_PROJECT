# FR-019 | locate active sessions on exit.

**Target requirement:** FR-019 — The system shall allow Parking Staff to locate active sessions on exit.
...
1. Primary feature screen for FR-019 with list/detail/workflow panel.
...
|---|---|---|
| POST | `/api/fr-019/records` | Create new record/configuration/workflow entry. |
| GET | `/api/fr-019/records` | List records with filters and pagination. |
| GET | `/api/fr-019/records/{recordId}` | Get record detail. |
| PUT | `/api/fr-019/records/{recordId}` | Update record fields. |
| PATCH | `/api/fr-019/records/{recordId}/status` | Change status/transition state. |
...
### Happy Path
1. **Given** valid permissions and required data exist, **when** the primary actor executes FR-019 workflow successfully, **then** the system persists results and returns the updated output.
