# FR-018 | create parking sessions with entry metadata.

**Target requirement:** FR-018 — The system shall allow Parking Staff to create parking sessions with entry metadata.
...
1. Primary feature screen for FR-018 with list/detail/workflow panel.
...
|---|---|---|
| POST | `/api/fr-018/records` | Create new record/configuration/workflow entry. |
| GET | `/api/fr-018/records` | List records with filters and pagination. |
| GET | `/api/fr-018/records/{recordId}` | Get record detail. |
| PUT | `/api/fr-018/records/{recordId}` | Update record fields. |
| PATCH | `/api/fr-018/records/{recordId}/status` | Change status/transition state. |
...
### Happy Path
1. **Given** valid permissions and required data exist, **when** the primary actor executes FR-018 workflow successfully, **then** the system persists results and returns the updated output.
