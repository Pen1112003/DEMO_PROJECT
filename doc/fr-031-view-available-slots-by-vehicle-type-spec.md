# FR-031 | view available slots by vehicle type.

**Target requirement:** FR-031 — The system shall allow Parking User/Driver to view available slots by vehicle type.
...
1. Primary feature screen for FR-031 with list/detail/workflow panel.
...
|---|---|---|
| POST | `/api/fr-031/records` | Create new record/configuration/workflow entry. |
| GET | `/api/fr-031/records` | List records with filters and pagination. |
| GET | `/api/fr-031/records/{recordId}` | Get record detail. |
| PUT | `/api/fr-031/records/{recordId}` | Update record fields. |
| PATCH | `/api/fr-031/records/{recordId}/status` | Change status/transition state. |
...
### Happy Path
1. **Given** valid permissions and required data exist, **when** the primary actor executes FR-031 workflow successfully, **then** the system persists results and returns the updated output.
