# FR-024 | correct wrong vehicle information.

**Target requirement:** FR-024 — The system shall allow Parking Staff to correct wrong vehicle information.
...
1. Primary feature screen for FR-024 with list/detail/workflow panel.
...
|---|---|---|
| POST | `/api/fr-024/records` | Create new record/configuration/workflow entry. |
| GET | `/api/fr-024/records` | List records with filters and pagination. |
| GET | `/api/fr-024/records/{recordId}` | Get record detail. |
| PUT | `/api/fr-024/records/{recordId}` | Update record fields. |
| PATCH | `/api/fr-024/records/{recordId}/status` | Change status/transition state. |
...
### Happy Path
1. **Given** valid permissions and required data exist, **when** the primary actor executes FR-024 workflow successfully, **then** the system persists results and returns the updated output.
