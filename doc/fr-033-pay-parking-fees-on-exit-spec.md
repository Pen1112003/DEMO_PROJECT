# FR-033 | pay parking fees on exit.

**Target requirement:** FR-033 — The system shall allow Parking User/Driver to pay parking fees on exit.
...
1. Primary feature screen for FR-033 with list/detail/workflow panel.
...
|---|---|---|
| POST | `/api/fr-033/records` | Create new record/configuration/workflow entry. |
| GET | `/api/fr-033/records` | List records with filters and pagination. |
| GET | `/api/fr-033/records/{recordId}` | Get record detail. |
| PUT | `/api/fr-033/records/{recordId}` | Update record fields. |
| PATCH | `/api/fr-033/records/{recordId}/status` | Change status/transition state. |
...
### Happy Path
1. **Given** valid permissions and required data exist, **when** the primary actor executes FR-033 workflow successfully, **then** the system persists results and returns the updated output.
