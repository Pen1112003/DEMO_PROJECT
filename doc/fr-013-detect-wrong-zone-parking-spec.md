# FR-013 | detect wrong-zone parking.

**Target requirement:** FR-013 — The system shall allow the Parking Manager to detect wrong-zone parking.
...
1. Primary feature screen for FR-013 with list/detail/workflow panel.
...
|---|---|---|
| POST | `/api/fr-013/records` | Create new record/configuration/workflow entry. |
| GET | `/api/fr-013/records` | List records with filters and pagination. |
| GET | `/api/fr-013/records/{recordId}` | Get record detail. |
| PUT | `/api/fr-013/records/{recordId}` | Update record fields. |
| PATCH | `/api/fr-013/records/{recordId}/status` | Change status/transition state. |
...
### Happy Path
1. **Given** valid permissions and required data exist, **when** the primary actor executes FR-013 workflow successfully, **then** the system persists results and returns the updated output.
