# FR-036 | pay for add-on services.

**Target requirement:** FR-036 — The system shall allow Parking User/Driver to pay for add-on services.
...
1. Primary feature screen for FR-036 with list/detail/workflow panel.
...
|---|---|---|
| POST | `/api/fr-036/records` | Create new record/configuration/workflow entry. |
| GET | `/api/fr-036/records` | List records with filters and pagination. |
| GET | `/api/fr-036/records/{recordId}` | Get record detail. |
| PUT | `/api/fr-036/records/{recordId}` | Update record fields. |
| PATCH | `/api/fr-036/records/{recordId}/status` | Change status/transition state. |
...
### Happy Path
1. **Given** valid permissions and required data exist, **when** the primary actor executes FR-036 workflow successfully, **then** the system persists results and returns the updated output.
