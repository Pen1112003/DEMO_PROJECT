# FR-014 | list unpaid vehicles.

**Target requirement:** FR-014 — The system shall allow the Parking Manager to list unpaid vehicles.
...
1. Primary feature screen for FR-014 with list/detail/workflow panel.
...
|---|---|---|
| POST | `/api/fr-014/records` | Create new record/configuration/workflow entry. |
| GET | `/api/fr-014/records` | List records with filters and pagination. |
| GET | `/api/fr-014/records/{recordId}` | Get record detail. |
| PUT | `/api/fr-014/records/{recordId}` | Update record fields. |
| PATCH | `/api/fr-014/records/{recordId}/status` | Change status/transition state. |
...
### Happy Path
1. **Given** valid permissions and required data exist, **when** the primary actor executes FR-014 workflow successfully, **then** the system persists results and returns the updated output.
