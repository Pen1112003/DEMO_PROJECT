# FR-030 | view pricing and parking rules.

**Target requirement:** FR-030 — The system shall allow Parking User/Driver to view pricing and parking rules.
...
1. Primary feature screen for FR-030 with list/detail/workflow panel.
...
|---|---|---|
| POST | `/api/fr-030/records` | Create new record/configuration/workflow entry. |
| GET | `/api/fr-030/records` | List records with filters and pagination. |
| GET | `/api/fr-030/records/{recordId}` | Get record detail. |
| PUT | `/api/fr-030/records/{recordId}` | Update record fields. |
| PATCH | `/api/fr-030/records/{recordId}/status` | Change status/transition state. |
...
### Happy Path
1. **Given** valid permissions and required data exist, **when** the primary actor executes FR-030 workflow successfully, **then** the system persists results and returns the updated output.
