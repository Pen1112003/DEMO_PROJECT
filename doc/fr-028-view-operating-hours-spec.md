# FR-028 | view operating hours.

**Target requirement:** FR-028 — The system shall allow Parking User/Driver to view operating hours.
...
1. Primary feature screen for FR-028 with list/detail/workflow panel.
...
|---|---|---|
| POST | `/api/fr-028/records` | Create new record/configuration/workflow entry. |
| GET | `/api/fr-028/records` | List records with filters and pagination. |
| GET | `/api/fr-028/records/{recordId}` | Get record detail. |
| PUT | `/api/fr-028/records/{recordId}` | Update record fields. |
| PATCH | `/api/fr-028/records/{recordId}/status` | Change status/transition state. |
...
### Happy Path
1. **Given** valid permissions and required data exist, **when** the primary actor executes FR-028 workflow successfully, **then** the system persists results and returns the updated output.
