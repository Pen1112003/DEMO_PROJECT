# FR-026 | handle wrong-zone parking exceptions.

**Target requirement:** FR-026 — The system shall allow Parking Staff to handle wrong-zone parking exceptions.
...
1. Primary feature screen for FR-026 with list/detail/workflow panel.
...
|---|---|---|
| POST | `/api/fr-026/records` | Create new record/configuration/workflow entry. |
| GET | `/api/fr-026/records` | List records with filters and pagination. |
| GET | `/api/fr-026/records/{recordId}` | Get record detail. |
| PUT | `/api/fr-026/records/{recordId}` | Update record fields. |
| PATCH | `/api/fr-026/records/{recordId}/status` | Change status/transition state. |
...
### Happy Path
1. **Given** valid permissions and required data exist, **when** the primary actor executes FR-026 workflow successfully, **then** the system persists results and returns the updated output.
